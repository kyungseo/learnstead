...
실행 한도 안에서도 시간과 토큰 사용량은 늘어날 수 있습니다. 다음 절에서는 그 변화를 측정합니다. `[문서 확인 · 2026-09-06]`
...
| 도구 | 구성 | 메인 컨텍스트(끝) | 메인 누적 입력 | 위임 누적 입력 | 총 경과 시간 | 위치 일치/3 |
| --- | --- | --- | --- | --- | --- | --- |
...
Codex 직접의 총 경과 시간은 첫 배치 값(19~29초)입니다. 재실행에서는 네트워크 시간 초과가 겹쳐 110초로 부풀었습니다(실습 VALIDATION).

이번 파일 5개 검토에서는 **메인 컨텍스트의 마지막 크기가 3~7% 줄었고, 메인과 위임을 합한 누적 입력은 늘었습니다.** 메인 컨텍스트의 대부분은 시스템 프롬프트·지시문·subagent 왕복이고, 파일 5개는 그 위의 몇천 토큰에 불과했습니다. 총 경과 시간도 Claude Code는 1.9배 늘었습니다. [context-engineering 05](../../guides/context-engineering/05-budget-and-compaction.md)의 "6,100 읽고 420 반환"은 읽을 것이 많을 때의 숫자입니다. 읽기를 맡기고 짧은 결과를 받는 방식은 메인에 남길 기록을 줄일 수 있습니다. 다만 파일 수나 토큰 수만으로 이득을 보장하는 경계값은 이 실험에서 구하지 않았습니다. 검토를 위임하기 전에 메인이 직접 읽을 때의 토큰 사용량을 측정하는 이유입니다. `[해석]`

## 5. 위임 요청만 읽어도 작업을 이해할 수 있어야 한다

대상 파일·요구사항·제약을 요청 안에 적어 이전 대화 없이도 작업을 이해할 수 있게 만드는 것을 **자기완결 프롬프트**라고 합니다. Claude Code의 fork가 아닌 subagent는 대화 이력을 못 받으므로 "우리가 확인한 그 결함"은 subagent에게 아무 뜻도 없습니다. Codex는 넘길 수도 있고 안 넘길 수도 있습니다. 실습 02는 메인이 결함을 확인한 뒤 **일부러 그 한 문장만으로** 위임하게 시킵니다. 관측할 것은 둘입니다.
...
Claude Code의 subagent는 모르는 것을 추측하지 않았고, 메인은 그 되물음을 받아 재위임할 수 있었습니다. Codex는 세 번 모두 수정했고 3회차에서 이력 전달(`fork_turns`)을 확인했습니다. 다만 이번 수집 기록에서는 **위임 메시지 본문을 읽을 수 없어 전달 내용을 전부 확인하지 못했습니다.** 같은 결함을 처음부터 **자기완결 프롬프트**(파일·함수·규격·검사 명령·건드리지 말 것)로 위임한 대조군과 비교하면 이렇습니다. 3회 중앙값. `[실행 검증 · 실습 02]`

| 도구 | 위임 프롬프트 | 수정 성공 | 메인 누적 입력 | 위임 누적 입력 | 총 경과 시간 |
| --- | --- | --- | --- | --- | --- |
