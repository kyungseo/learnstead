# 06. 실패 지도 — 나눴는데 틀리는 다섯 가지

> 이전 ← [`05-plan-execute-review.md`](05-plan-execute-review.md) · 다음 → [`07-when-not-to-split.md`](07-when-not-to-split.md)

## 이 장에서 답하는 질문

- 코딩 에이전트에서 나누기가 실패할 때 어떤 모양으로 나타나는가
...
[코드 레벨 가이드 04](../../guides/agent-orchestration/04-failure-modes.md)의 다섯 실패(오분류 전파·수렴 실패·조용한 손실·비용 폭발·책임 불명)는 전부 "정상 동작처럼 보이는 실패"였습니다. 코딩 에이전트의 나누기도 같습니다. 아래 다섯은 이 가이드의 실측과 두 도구의 문서에서 모은 것이고, 일부는 명시적인 오류 없이 끝납니다. merge 충돌이나 테스트 실패처럼 오류로 드러나는 경우도 있습니다. `[해석]`

| # | 실패 | 증상 | 이 가이드의 근거 |
| --- | --- | --- | --- |
| ① | **위임 프롬프트 결핍** | subagent가 엉뚱한 것을 고치거나 "무엇을 하라는지 모르겠다"고 돌아온다. 이력을 넘기는 도구에서는 비용이 늘고 무엇을 넘겼는지 알 수 없다 | [03 §5](03-subagent-anatomy.md) — 비-fork subagent는 대화 이력을 못 받는다 |
...
| ④ | **검토 과잉** | "빠짐없이 찾아라"가 낸 골든셋 밖 지적을 전부 반영해 코드가 불어난다 | [05 §4](05-plan-execute-review.md) |
| ⑤ | **비용 폭주** | 메인 컨텍스트는 그대로인데 전체 토큰이 배, 팀은 7배 | [03 §4](03-subagent-anatomy.md), [02 §4](02-layers-of-splitting.md) |
...
| ④ | 검토 finding 목록 vs 골든셋·규격 | 규격에 없는 지적의 비율. 타당한지는 사람이 판정 |
| ⑤ | Claude Code `usage`(메인)와 `modelUsage`(전체), Codex `turn.completed`와 스레드별 `token_count` | 메인 대비 전체 배수 |

[MCP 가이드 07](../../guides/mcp-basics/07-what-goes-wrong.md)의 "모델의 말은 근거가 아니다 — 로그의 호출 줄을 본다"가 나누기에서는 두 배로 중요합니다. 메인이 "subagent가 확인했다"고 말해도, subagent의 transcript가 근거입니다. Codex CLI의 `exec --json` 스트림은 `wait` 호출만 보여 주고 `spawn_agent`를 보여 주지 않았습니다. 스트림에 없다고 안 한 것이 아니고, 세션 rollout을 봐야 했습니다. `[실행 검증 · 2026-09-06]`
...
| ④ | finding의 정의를 프롬프트에, 검사 통과를 검토자 앞에 | `/code-review`도 correctness 위주 |
...
나누기가 늘어날수록 "누가 무엇을 허락했나"가 흐려집니다. 도구가 적용하는 제한과 모델에게 주는 지침을 구분해 봅니다. `[문서 확인 · 2026-09-06]`
...
[MCP 가이드 05](../../guides/mcp-basics/05-permission-boundaries.md)의 "경계를 한 곳에만 두면 다른 도구로 샌다"가 나누기에서는 "subagent에 경계를 두면 메인으로 샌다"가 됩니다. 검토용 subagent를 읽기 전용으로 만들어도 메인이 쓰기 권한이면 메인이 씁니다. 경계는 **가장 넓은 권한을 가진 세션** 기준으로 봅니다. `[해석]`
...
- 멈춤 장치를 프롬프트 규율·격리·예산으로 나눠 설치할 수 있습니다.
