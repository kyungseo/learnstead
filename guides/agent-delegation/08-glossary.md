...
| 위임 프롬프트 결핍 | 이력을 못 받는 subagent(Claude Code 비-fork)에 이력에 기대는 프롬프트를 넘겨 생기는 실패. 이력을 넘기는 도구에서는 비용과 감사 불가로 나타난다 | 이 가이드 06 |
| 조용한 손실 | 워커는 맞았는데 취합에서 빠지거나 뒤집히는 실패 | [코드 레벨 가이드 04](../../guides/agent-orchestration/04-failure-modes.md) |
...
| 기준선 | 나누기 전 한 세션의 토큰·시간·적중 | 이 가이드 01 |
...
| worktree vs 파일 소유권 | 작업 폴더 분리 ↔ 각 작업의 쓰기 범위 지정. 둘 다 통합 검증은 필요 |
