...
- 서브에이전트가 좋다는 진짜 이유는 무엇이고 그것을 어떤 숫자로 확인하는가
...
- **컨텍스트 격리와 토큰 절감은 다릅니다.** 워커 입력을 나눠도 위임·취합 때문에 전체 토큰이 늘 수 있습니다. `[해석]`
...
- **워커가 늘어난 이유:** 워커 셋이 각자 문서를 보고(3×), 상위가 결과 셋을 다시 봤습니다(+1).
- **루프가 늘어난 이유:** 평가자가 문서 전체 + 초안을 봐야 해서, 한 라운드에 문서를 두 번 읽는 셈이 됐습니다.
...
`[해석]` — 별도 호출이라는 구조, 전달 정보의 범위, 전체 비용을 서로 다른 질문으로 봅니다.

## 4. 줄이는 세 가지 방법

1. **입력을 고르기** — 분류·검색으로 "볼 것"을 먼저 좁힙니다(라우터·[RAG 가이드](../../guides/local-rag/README.md)).
2. **결과만 올리기** — 하위는 원문을 보고, 상위는 **요약된 결과만** 받습니다.
3. **기록을 잘라 넘기기** — 단계 사이에 대화 기록 전체를 넘기지 않습니다. 필요한 값만 넘깁니다
...
- 입력 고르기·결과만 올리기·기록 잘라 넘기기로 내 구성의 토큰을 줄일 수 있습니다.
