...
- 멀티 구성의 실패 다섯 가지는 무엇이고 왜 오류 메시지만으로 알아보기 어려운가
...
- 멈춤 장치는 셋 — **상한(횟수·시간·토큰) · 폴백 · 단계별 로그.** 셋 다 코드에 있어야 합니다. `[해석]`
...
상한을 agent 루프 코드에 넣는 방법 자체는 [앱 연결 06](../../guides/local-llm-app-integration/06-tool-calling-workflow-agent.md)의 최소 루프와 [앱 연결 10 §5](../../guides/local-llm-app-integration/10-reliability-and-operations.md)의 운영 수칙에 있습니다. 여기서는 멀티 구성에서 세 상한이 각각 어느 실패를 막는지만 봅니다.
...
멀티 구성에서 사람이 봐야 하는 곳은 셋입니다. `[해석]`

1. **분기 판단** — 라우터가 고른 분야가 맞는지 (신뢰도가 낮을 때만 물어도 됨)
2. **상한 도달** — 자동으로 대충 끝내지 말고 알림
3. **쓰기·전송 직전** — 도구가 실제로 뭔가를 바꾸기 전 ([앱 연결 06 §3](../../guides/local-llm-app-integration/06-tool-calling-workflow-agent.md))
