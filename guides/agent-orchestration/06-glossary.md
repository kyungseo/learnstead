...
| **오케스트레이션 (orchestration)** | 여러 호출·agent를 엮어 하나의 일을 완성하는 것 |
| **오케스트레이터 (orchestrator)** | 일을 쪼개 뿌리고 결과를 합치는 상위 구성 요소 ([실습 02](../../labs/when-splitting-fails/02-orchestrator-workers.md)) |
| **워커 (worker)** | 쪼개진 일 하나를 맡는 하위 구성 요소 |
...
| **팬아웃/팬인 (fan-out / fan-in)** | 여럿에게 뿌리기 / 결과를 모으기. 고정 작업 분배에도 쓰이는 흐름 |
...
| **기준선 (baseline)** | 단일 호출로 만든 비교 기준. 나눈 구성은 이것과 비교 ([05 §4](05-when-not-to.md)) |
...
| **나누기 vs 격리** | 여러 번 부르기 ↔ 각자 덜 보게 하기. 입력 경계와 전체 비용을 별도로 확인 |
