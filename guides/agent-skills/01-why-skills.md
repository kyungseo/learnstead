...
`CLAUDE.md`·`AGENTS.md`는 **항상** 읽힙니다. 여기에 긴 절차를 쌓으면 모든 대화가 그만큼 무거워지고, 관련 없는 작업에서도 그 절차가 모델의 주의를 끕니다. skill은 반대입니다. 평소에는 **이름과 짧은 설명만** 컨텍스트에 있고, 본문은 호출될 때만 들어옵니다. Agent Skills 규격은 이를 **progressive disclosure(점진적 공개)라고** 부르며, 단계별 크기 지침을 제공합니다 [문서 확인].
...
| hook | 이벤트에 묶인 shell 명령 | 설정된 이벤트·조건에서 런타임이 실행한다. skill은 모델이 읽고 따르는 지시다 |
