...
| `description` | 1~1024자, 비어 있으면 안 됨 | **이름과 함께 초기 목록에 들어가는 정보.** 모델이 "이 skill을 지금 쓸까"를 판단하는 근거 |
...
- **출력 형식을 실제 예로**: "표로 정리해"라고만 쓰는 것보다 표 뼈대 한 줄을 보여 주면 기대 형식을 구체적으로 전달할 수 있습니다.
...
- **예 한 쌍**: 입력과 기대 출력을 함께 보여 줍니다. 규칙이 실제 입력과 출력에 어떻게 적용되는지 보여 줍니다.
...
스크립트 경로는 도구가 치환 변수를 제공하면 그것을 사용합니다. Claude Code는 `${CLAUDE_SKILL_DIR}`를 skill 폴더 절대 경로로 바꿔 줍니다 [문서 확인]. 규격 자체에는 그런 변수가 없으므로, 여러 도구용 skill이면 "이 skill 폴더의 `scripts/check.py`"처럼 **폴더 기준 상대 경로를 말로** 적는 편이 안전합니다. 실습의 Codex 실행에서는 모델이 `sed -n '1,240p' .agents/skills/meeting-actions/SKILL.md`로 파일을 직접 읽었습니다 [실행 검증]. 이 실행의 shell 상대 경로는 작업 디렉터리 기준이었습니다. skill의 부속 파일은 skill 폴더를 기준으로 찾도록 명시합니다.
