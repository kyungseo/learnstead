...
| **내용물** | 무엇이 들어 있는가 | 지시문(모델이 읽고 따름) · 코드(이벤트·조건에 따라 실행) · 능력(새 tool) · 묶음(여럿의 패키지) |
...
hook은 모델이 판단하지 않습니다. "commit 전에 lint를 돌려라"를 skill에 쓰면 모델이 잊을 수 있지만 해당 명령을 검사하는 hook을 올바르게 설정하면 모델의 기억에 의존하지 않고 실행할 수 있습니다. hook의 적용 범위 밖에서 실행한 명령까지 검사하는 것은 아닙니다. Claude Code 문서도 "skill이 첫 응답 이후 무시되는 것 같으면 hook으로 결정적으로 강제하라"고 안내합니다 [문서 확인]. 둘은 경쟁 관계가 아니라 서로 보완합니다. **절차는 skill에, 강제는 hook에 둡니다.**
