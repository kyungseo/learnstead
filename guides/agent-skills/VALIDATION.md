...
- 아래 실행 결과는 같은 fixture로 2026-08-30에 확인한 기록이며, 이번 점검에서 다시 실행한 결과는 아닙니다.

## 2026-09-03 공개 전 재검증

- 현재 설치본 Claude Code 2.1.259와 Codex CLI 0.144.1로 같은 이름의 skill 충돌 경로를 다시 실행했습니다.
- 실험용 이름을 `ws-dup-meeting-actions`로 격리한 상태에서 Claude Code는 개인 사본, Codex는 저장소 사본을 선택했습니다. 각 도구에서 1회씩 확인했으므로, 이 결과만으로 항상 같은 사본이 선택된다고 볼 수는 없습니다.
