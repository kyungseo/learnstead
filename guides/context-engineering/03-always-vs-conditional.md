...
- **pointer만으로 자동 조건부 로드가 보장되지는 않습니다.** 모델이 읽을지 말지를 판단하고, 읽어도 "지시"보다 "참고"로 취급하는 경향이 보였습니다. Claude Code에서 pointer(9)와 import(13)의 차이가 그것입니다. 조건부로 넣고 싶으면 도구가 제공하는 조건부 기제(경로 규칙·skill)를 씁니다.
...
- Codex: transcript에 `sed … src/AGENTS.md` 같은 **파일 읽기 줄이** 보이면 모델이 명시적으로 읽은 사실을 확인할 수 있습니다. 그 줄만으로 자동 로드 여부까지 판정하지는 않습니다. 자동 로드된 지시문은 transcript에 나타나지 않는다 [실행 검증].

## 이 장을 끝내면

- 조건부 로드 네 형태와 각각의 켜지는 조건을 도구별로 말할 수 있습니다.
- pointer만으로 자동 로드가 보장되지 않음을 실측과 도구의 로드 규칙으로 설명할 수 있습니다.
