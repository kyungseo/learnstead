...
[← 이전: 02. 내 앱에는 어떤 DB가 필요할까?](02-choose-a-database.md) · [목차](README.md) · [다음: 04. 앱은 DB에 어떻게 요청할까?](04-talk-to-the-database.md)
<!-- learnstead:nav:end -->
...
이 장의 결과는 **사용자·모임·신청의 관계를 작은 그림으로 설명하는 것입니다**. 첫 설계에서는 이름·날짜 같은 저장 항목을 늘리기 전에, 어떤 기록들을 따로 관리해야 하는지 먼저 생각해 봅니다.

<a id="신청은-사람도-모임도-아니다"></a>

## 사용자·모임·신청을 따로 기록하는 이유

‘연습 사용자 A가 독서 모임에 신청했다’는 상황에는 사용자, 모임, 신청이라는 세 종류의 정보가 있습니다. 사용자는 다른 모임에도 신청할 수 있고, 모임에는 다른 사용자도 신청할 수 있습니다. 그래서 사용자와 모임을 따로 보관하고, 누가 어느 모임에 신청했는지를 신청 기록으로 연결합니다.

같은 종류의 기록을 모은 표를 **테이블이라고 합니다**. 한 줄인 행은 모임 하나나 신청 한 건을, 세로 항목인 열은 제목·정원 같은 정보를 담습니다. 이런 항목을 필드라고도 부릅니다. ID는 기록마다 붙이는 고유한 식별자입니다. 이름이 같거나 바뀌어도 같은 기록인지 알아볼 수 있도록 사용합니다. 사람 이름이나 화면의 줄 번호는 바뀌거나 중복될 수 있어 식별자로 쓰기 어렵습니다.

아래에서 `events`와 `registrations`는 각각 모임과 신청을 보관하는 테이블 이름입니다. 신청 기록의 `event_id`와 `user_id`는 어느 모임에 누가 신청했는지 연결하는 값입니다.
...
신청 기록에는 사용자와 모임을 연결하는 ID 외에도 신청 메모나 신청 시각을 담을 수 있습니다. 같은 사용자가 여러 모임에 신청해도, 메모와 시각을 신청 건마다 따로 관리할 수 있습니다.
