...
이 가이드는 코드를 깊이 읽기 어려운 사람도 **앱이 데이터를 다루는 방식을 설명하고 확인하도록** 돕습니다. SQLite·PostgreSQL·MySQL·MongoDB·Cloud Firestore를 비교하고, 저장 구조·접근 권한·변경·복구를 설명합니다.

공통 예시는 **스터디 모임 신청 앱입니다**. 사용자가 모임을 보고 참가 신청을 하거나 자신의 신청을 취소하는 작은 앱입니다. 이 앱에 어떤 정보를 저장하고, 다른 사람의 신청을 바꾸지 못하게 하려면 어떻게 해야 하는지 차례로 살펴봅니다. 개념 가이드를 읽을 때는 앱을 설치하거나 실행할 필요가 없습니다. 각 장의 확인 활동과 프롬프트는 앱을 만들었을 때 활용할 수 있습니다.
...
저장 위치만 궁금하다면 [01장](01-where-data-lives.md)을 읽고, DB를 고르는 기준까지 알고 싶다면 [02장](02-choose-a-database.md)으로 이어 가세요.

이미 만든 앱을 공개하기 전에 점검하려면 **01장 → 03장 → 06장 → 09장을** 읽고 [데이터 관리 카드](DATA-CARD.md)를 채워 보세요. 09장의 표에서 데이터 규칙·변경·복구가 낯설다면 연결된 05·07·08장을 함께 읽습니다. 처음부터 차근차근 배우려면 아래 목차 순서대로 읽습니다.
...
`문서 확인 · 2026-09-13`: 제품 기능은 [공식 출처](SOURCES.md)를 확인했습니다. MySQL·MongoDB·Firestore의 설치와 운영은 이 시리즈에서 직접 실행한 경로가 아닙니다. Supabase는 로컬 환경에서만 실행했으며, 클라우드 운영은 검증하지 않았습니다. 정확한 실행 범위는 [검증 기록](VALIDATION.md)에서 구분합니다.
