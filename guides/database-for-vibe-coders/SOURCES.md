...
메모리·화면·저장소의 구분, 데이터 구조와 변경 절차, 코드·구조·데이터 복원의 구분은 원리를 설명한 것입니다. 제품별 CLI의 실행 증거가 아닙니다. 이 시리즈에서는 스터디 신청 앱으로 데이터 사이의 관계를 배우기 위해 관계형 DB를 선택했습니다. 다른 제품보다 항상 더 낫다는 뜻은 아닙니다.
