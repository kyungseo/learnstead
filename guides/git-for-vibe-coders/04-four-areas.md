...
- 여러 오류는 "지금 어느 칸에 있는 파일 얘기인가"만 알아도 풀립니다. `[해석]`
...
| `git add .` | ① → ② | 현재 폴더 아래의 변경을 담겠다 (ignore 대상 제외) |
...
| `git restore 파일` | ② → ① | 해당 파일의 add 이후 변경을 버리고 staging 상태로 되돌린다 ([05](05-commits-and-undo.md)) |
...
한 커밋에 관련 없는 변경이 뒤섞이면 나중에 한 가지 변경만 골라 되돌리기 어려워집니다. **커밋 하나 = 의미 하나가** 원칙이고,
스테이징은 그것을 가능하게 하는 장치입니다. `[해석]`

> `git add .`은 현재 폴더 아래의 변경을 함께 담습니다. 실행 전에 파일 목록과 diff를 확인하세요. 다만 **비밀번호 파일까지 함께 담기는 사고가** 여기서 납니다([11 S10](11-scenarios-share.md)).
...
안 담깁니다. 단, 이미 Git이 추적 중인 파일에는 소급 적용되지 않습니다. 그 경우에는 [11 S10](11-scenarios-share.md)의 단계별 처리가 필요합니다. `[원리]`
