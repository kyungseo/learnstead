...
> **왜 읽나:** AI와 코딩할 때 변경을 저장하고 되돌리는 방법을 다룹니다. 그리고 되돌리기 명령 중 **하나는 되돌리기가 아니라
...
- `git reset --hard`는 **추적 중인 파일의 미커밋 변경을 버리며, Git으로 복구하지 못할 수 있습니다.** 실제로 확인했습니다(§4). `[실행 검증 · 2026-08-23]`
...
| 최근 커밋들을 통째로 없던 일로 | `git reset --hard 해시` | 브랜치를 지정 커밋으로 옮기고 스테이징·작업 파일을 맞춤. 미커밋 수정 손실 위험 | ⛔ 위험 (§4) |
...
| **기록에 담지 않은 수정** | tracked file의 변경을 잃을 수 있으며 Git만으로 복구가 보장되지 않음 |
...
일반적인 untracked 파일을 전부 지우는 명령은 아니지만, 대상 commit의 파일을 펼치는 데 방해되는 untracked 경로는 지워질 수 있습니다. `--hard` 실행 전에 추적 여부와 관계없이 보존할 작업을 확인합니다.
...
`restore`는 **지목한 파일만** 건드립니다. 범위는 좁지만 그 파일의 미커밋 변경은 사라집니다. 기본 복원 기준은 staging입니다. add하지 않은 상태라면 마지막 commit과 같지만, add한 뒤 다시 수정했다면 add 이후 변경만 버립니다. 먼저 `git diff -- index.html`로 버릴 내용을 확인합니다.
...
- [ ] `reset --hard`가 추적 파일의 미커밋 변경을 버리며 복구를 보장하지 않는다는 것을 안다
