...
- 브랜치는 **"같은 프로젝트의 평행 세계"입니다.** 파일을 복사하지 않고, 오갈 때 추적 파일이 해당 branch의 상태에 맞춰집니다. 미커밋 변경이 함께 따라가거나 전환이 거부될 수도 있습니다. `[원리]`
...
대표적인 경우는 **같은 파일의 같은 부분을** 다르게 고쳤을 때입니다. 한쪽의 삭제와 다른 쪽의 수정처럼 다른 형태의 충돌도 있습니다. 충돌 없이 합쳐져도 원하는 동작인지 확인해야 합니다. `[원리]`
...
> 진행 중인 병합을 중단하려면 `git merge --abort`를 사용합니다. 다만 병합 전부터 미커밋 변경이 있었다면 원상 복구가 어려울 수 있으므로, 병합 전에 작업을 보존합니다. `[문서 확인 · 2026-08-23]`
...
합친 브랜치는 지웁니다: `git branch -d 이름` (합치지 않은 브랜치를 강제로 지울 때는 `-D`). 브랜치가 쌓이면
각각의 목적과 남길 변경이 있는지 확인합니다. 개수만으로 잘못된 작업 흐름이라고 판단하지는 않습니다. `[해석]`
