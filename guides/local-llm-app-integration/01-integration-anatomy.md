...
- 연결 = 내 프로그램이 runtime의 HTTP endpoint에 **JSON을 POST**하고 **JSON을 받는** 것. 특별한 프로토콜이 아닙니다. `[원리]`
- 주요 로컬 runtime은 **OpenAI 호환 형식**을 제공합니다. 기본 채팅 코드는 재사용하기 쉽지만, 전환할 때는 `base_url`·`model`·키와
  가장자리 기능을 함께 확인합니다. `[문서 확인 · 2026-08-24]`
- 연결 방식은 네 가지 — **직접 SDK, 프레임워크, 게이트웨이 경유, in-process 바인딩** — 이고, 처음엔 직접 SDK가 맞습니다. `[해석]`
...
구현할 수 있고, 호환 범위 안에서는 client code를 재사용할 수 있습니다. backend를 바꿀 때는 주소·모델·키와 지원 기능을 다시 검증합니다.
...
반대로 "챗 하나, 도구 두 개"라면 프레임워크 없이 끝나는 것이 보통 더 낫습니다. `[해석]`

## 3. runtime별 차이 — 기본 경로는 같고 가장자리는 다르다
...
`[문서 확인 · 2026-08-24]` — 포트는 기본값이며 바꿀 수 있습니다. "호환"의 범위는 runtime마다 달라서 **기본 채팅 경로는
비슷하지만, 구조화 출력·tool calling·스트리밍 세부·context 설정은 다릅니다.** 차이표는 [02 §4](02-openai-compatible-api.md).
...
Mistral 계열 등이 있으며, Gemma 3는 Ollama에서 tool 템플릿을 제공하지 않습니다. 이 가이드는 **대화·구조화 출력에는
`gemma3:4b`, tool calling에는 `qwen3:4b`**를 예시로 씁니다. 태그와 지원 여부는 바뀌므로 실행 전 `ollama show`로 확인합니다.
