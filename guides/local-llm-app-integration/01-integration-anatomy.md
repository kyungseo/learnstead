...
- 이 가이드의 Chat Completions 호출은 이전 대화를 자동으로 이어 주지 않습니다(stateless). **내 프로그램이** 기록을 보관하고 필요한 부분을 매번 보냅니다. `[원리]`
...
> **초심자용 한 줄:** 모델 호출은 "웹 API 호출"입니다. 날씨 API처럼 HTTP 요청과 응답을 주고받습니다. 다만 생성된 답은
> 매번 달라질 수 있어, 응답 형식뿐 아니라 내용과 도구 실행 권한도 별도로 검증해야 합니다.
