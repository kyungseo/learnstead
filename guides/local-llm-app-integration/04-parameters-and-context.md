...
| `seed` | 난수 시드 | 테스트·재현 시 | runtime·하드웨어 등 조건을 맞춰도 동일 출력은 보장되지 않음 |
...
![프롬프트(시스템 규칙 + 대화 기록 + 질문)와 답변 여지가 runtime이 적용한 context 창 안에 들어가야 하며, 초과 입력의 처리는 runtime·endpoint에 따라 달라지는 모습](diagrams/04-context-budget.svg)
...
| **토큰 예산 자르기** | 전송 전에 tokenizer로 추정해 기록을 줄이고, 응답의 `usage.prompt_tokens`로 추정을 보정 | 오래된 정보 | 메시지 길이가 들쭉날쭉할 때 |
...
- [ ] 내 앱의 한 요청 크기(system + 기록 + 입력 + max_tokens)가 답변 여유를 포함해 적용 창 안에 들어가는지 계산했다
