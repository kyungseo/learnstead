...
대화 프로그램의 출력은 사람이 읽는 문자열입니다. 프로그램에서 필드별로 다루기 쉽게 **정해진 모양의 JSON으로** 받을 수 있습니다. "JSON으로 답해"라고
프롬프트로 지시하는 것과, runtime이 **토큰 단위로 스키마를 강제**하는 것은 다른 일입니다. 이 문서는 그 차이와 사용법을 다룹니다.
...
- 프롬프트로 "JSON만"이라고 해도 모델은 코드 펜스·설명문·누락 필드를 섞습니다. 정해진 형식이 필요하면 **runtime의 스키마 제한을** 사용해야 합니다. `[해석]`
...
- 제한하는 것은 **형식입니다.** 값이 맞는지는 검증 코드로 원문·범위와 대조하고, 자동 판정이 어려운 내용은 사람이 확인합니다. `[원리]`
- 스키마는 작게, `required`는 명확하게, "없으면 빈 값"을 프롬프트에 적습니다. `[해석]`

<a id="1-왜-부탁으로는-안-되는가"></a>

## 1. 프롬프트만으로 형식을 지키기 어려운 이유

![프롬프트로만 JSON을 요청했을 때 코드 펜스·설명문·필드 누락이 섞여 파싱이 실패하는 경로와, 스키마 제한 뒤 파싱·내용 검증까지 거치는 경로의 비교](diagrams/05-structured-output.svg)

`[원리]` 모델은 확률적으로 토큰을 고르므로 "항상 JSON만"을 100% 지키지 못합니다. 스키마 강제는 생성 단계에서 **문법에 맞지 않는 토큰을 배제**해 형식 실패 가능성을 크게 줄입니다(grammar-constrained decoding). 앱은 응답이 비었거나 잘린 경우까지 처리해야 합니다.
...
제목을 만들었고, 자신감은 0.9였습니다. 구조화 출력은 형식만 보장하며, **모델의 자기 평가도 정확성을 보장하지 않습니다.**
따라서 §4에서 설명하듯 `confidence` 문턱만 사용하지 말고 원문과 결과를 대조해야 합니다. `[실행 검증 · 2026-08-23]`
...
| (3) `json.loads` | 문자열을 JSON으로 파싱 | 실패하면 빈 응답·잘림·지원 스키마·호환성을 확인 |
...
| 원칙 | 이유 |
| --- | --- |
| **스키마는 작게** | 필드가 많을수록 모델이 원문에 없는 값을 채울 가능성이 커짐. 꼭 필요한 필드만 사용 |
| **"없으면 빈 값"을 명시** | 빈 값 처리 규칙이 없으면 모델이 값을 임의로 채울 수 있음. 프롬프트와 `description`에 규칙을 명시 |
| **enum으로 선택지 고정** | `"status": {"enum": ["승인", "반려", "보류"]}` — 자유 문자열보다 안정 |
| **confidence 또는 근거 필드** | 값이 낮으면 사람이 확인하도록 연결하되, 모델의 자기 평가만으로 정확성을 판정하지 않음 |
