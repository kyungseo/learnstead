...
모델이 계산 결과를 생성할 수는 있지만, 외부 계산기·검색·파일 읽기 도구를 **직접 실행하지는 않습니다.** 모델은 "이 함수를 이 인자로 불러 달라"는 **요청만 출력**하고,
실행은 내 프로그램이 담당합니다. 이 차이를 이해하면 tool calling·workflow·agent를 구분할 수 있고, 권한 경계를 왜 내 코드에
두어야 하는지도 알 수 있습니다.


> **왜 읽나:** 모델의 텍스트 생성과 외부 도구 실행은 별개의 과정입니다. "이 함수를 불러 달라"고 말할 수 있을 뿐이고, 그 말을 누가 어디까지 들어줄지는 내 코드가 정합니다.
>
...
tool calling은 모델의 도구 호출 지원과 runtime의 템플릿·파서가 맞아야 동작합니다. `ollama show <model>`의 `Capabilities`에
