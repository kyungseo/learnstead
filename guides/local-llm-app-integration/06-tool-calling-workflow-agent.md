...
모델은 계산도, 검색도, 파일 읽기도 **직접 하지 못합니다.** 모델이 할 수 있는 것은 "이 함수를 이 인자로 불러 달라"는 **요청을
출력**하는 것뿐이고, 실행은 내 프로그램이 합니다. 이 구분을 잡으면 tool calling·workflow·agent의 차이와, 왜 권한 경계가 내
코드에 있어야 하는지가 한 번에 보입니다.
...
- **tool calling** = 모델이 "함수 이름 + 인자 JSON"을 구조화 출력으로 내놓으면, 내 코드가 실행하고 결과를 돌려주는 왕복. 모델은 실행하지 않습니다. `[원리]`
- **workflow** = 호출 순서를 **내 코드가** 정한 것. **agent** = 다음 호출을 **모델이** 정하는 루프. 차이는 "누가 제어 흐름을 쥐는가"입니다. `[해석]`
...
> **초심자용 한 줄:** tool calling은 모델에게 "메뉴판"(도구 명세)을 보여 주고, 모델이 "이거 주세요"(호출 요청)라고 하면 **주방(내
> 코드)**이 만들어 주는 것입니다. 모델은 주방에 들어가지 못합니다.
...
tool calling은 모델이 그 형식을 **학습**했어야 동작합니다. runtime은 형식을 변환할 뿐입니다. `ollama show <model>`의 `Capabilities`에
...
`[해석]` 흔한 실수는 workflow로 충분한 일을 agent로 만드는 것입니다. 순서를 아는 일은 코드가 순서를 쥐는 편이 싸고 안전합니다.
agent는 **"다음에 무엇을 할지 미리 모르는"** 일에만 값어치가 있습니다.
...
## 4. MCP — 도구 연결의 표준화 (한 줄 감각)

도구 명세와 호출 형식을 앱마다 다르게 만들지 않도록, 도구 서버와 모델 호스트 사이의 프로토콜을 표준화한 것이 **MCP(Model
Context Protocol)**입니다. 이 가이드의 도구는 Python 함수 네 개라 MCP가 필요 없지만, 같은 도구를 여러 앱·에디터에서 쓰려면
