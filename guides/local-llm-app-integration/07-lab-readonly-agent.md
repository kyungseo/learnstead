...
> **왜 읽나:** 실습에서 모델은 첫 검색에 실패하자 스스로 목록을 보고 문서를 통째로 읽어 답을 찾았습니다. 그 "다음 행동을 고르는 것"이 agent의 전부입니다.
>
...
**경계가 어디 있는지** 세 곳을 찾아 두세요. 08에서 하나씩 건드립니다.
...
모델은 계산기를 **한 번만** 불렀고, "40일 → 960시간"으로 이어가지 않은 채 단위가 없다고 되물었습니다. 틀린 답은 아니지만
기대와 다릅니다 — 도구를 **몇 번, 어디까지 쓸지도 모델이 정한다**는 것이 첫 관찰입니다. "40일을 시간으로 바꿔서"라고
...
세 줄짜리 작은 드라마입니다. 모델이 고른 첫 키워드 "연차 이월"은 문서에 그 **띄어쓰기 그대로**는 없어서(문서는 "연차는 …
이월해") 검색이 빗나갔고, 모델은 포기하지 않고 목록을 보고 문서를 통째로 읽어 답을 찾았습니다. 이것이 agent입니다 —
첫 시도가 실패하면 **다음 행동을 모델이 고릅니다.** 같은 질문을 두세 번 실행해 경로가 달라지는지 보세요. `[실행 검증 · 2026-08-23]`
...
답하면 정상입니다. 작성 환경에서는 더 영리하게, 읽기 전에 **`list_docs`부터 호출**해 목록에 없음을 확인하고 "출장 규정 문서가
...
총 호출도 기본 12회에서 멈춥니다. `[부분 검증 · 2026-08-24]` 상한은 "답을 못 해도 멈춘다"는 약속입니다.
...
**다음 →** [08 실습: Prompt Injection과 도구 권한 경계 — 경계를 일부러 넘어 보기](08-lab-prompt-injection.md)
