...
여기까지는 `openai` SDK 하나로 전부 했습니다. 이 문서는 **언제 프레임워크로 넘어가는지**, 그리고 호출 하나를 **앱**으로 키울 때
필요한 구조 — 스트리밍 UI, 동시성, 캐시, 그리고 로컬에서 개발해 다른 백엔드로 가는 전환 — 를 다룹니다.

← [08 실습: Prompt Injection](08-lab-prompt-injection.md) · 다음 → [10 신뢰성과 운영](10-reliability-and-operations.md)

> **왜 읽나:** 프레임워크는 부품을 주고 투명성을 가져갑니다. 챗 하나에 도구 몇 개라면 SDK가 낫고, 체인이 세 단을 넘으면 얘기가 달라집니다.
>
...
- 프레임워크는 **부품(커넥터·체인·메모리·도구 추상화)**을 주고 **투명성**을 가져갑니다. 챗 하나 + 도구 몇 개는 SDK로 충분합니다. `[해석]`
- Python은 LangChain·LlamaIndex·Haystack, Java는 Spring AI·LangChain4j, JS는 Vercel AI SDK가 2026년 8월 기준 대표적입니다. `[자료 확인 · 2026-08-23]`
- 앱 구조의 핵심 결정은 셋 — **스트리밍을 UI까지 흘릴 것인가, 동시 요청을 어떻게 받을 것인가, 백엔드 전환점을 어디에 둘 것인가.** `[해석]`
- 로컬→운영 전환은 `base_url`·`model`·키를 **설정으로 빼는 것**에서 시작하고, 여러 백엔드면 게이트웨이 층을 둡니다. `[해석]`
...
| 3. 가장자리 기능 확인 | 구조화 출력·tool calling이 새 백엔드에서도 같은 방식으로 지원되는지 ([02 §4](02-openai-compatible-api.md)) |
...
`base_url`·`model`·키뿐 아니라 지원 기능과 응답 차이를 전환 테스트로 확인해야 합니다.
