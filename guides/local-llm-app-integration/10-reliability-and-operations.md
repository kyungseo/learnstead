...
>
> **읽고 나면:** 네 가지 실패 범주를 구분해 대응하고, 재시도할 오류와 재시도하지 않을 오류를 나누며, 로그·골든셋·모킹으로 변경 전후의 품질을 비교할 수 있습니다.
>
...
- 실패는 **연결 실패, 요청 거부, 잘린·빈 응답, 성공했지만 틀린 응답** 등 네 범주로 나눌 수 있습니다. 연결 실패와 요청 거부는 예외로 드러나지만, 나머지는 코드가 응답을 직접 검사해야 찾을 수 있습니다. `[원리]`
- 재시도는 **연결 오류와 일시 오류에만** 적용하고 횟수 상한을 둡니다. 틀린 답은 같은 요청을 다시 보낸다고 근본적으로 해결되지 않습니다. `[해석]`
- 로그는 기본적으로 **모델·파라미터·길이·해시·finish_reason·usage·지연**을 남깁니다. 본문은 마스킹·접근 통제·보존 기간을
  정한 디버그 환경에서만 제한적으로 기록합니다. `[해석]`
- 이 실습의 Ollama 로컬 endpoint는 기본적으로 인증을 요구하지 않습니다. `0.0.0.0`으로 열지 말고, 열어야 하면 앞에 인증 프록시를 둡니다. `[문서 확인 · 2026-08-23]`
...
| **느린 첫 응답** | 첫 토큰까지 수십 초 | 지연 측정 | 모델 적재·긴 입력·대기열 확인. 적재가 원인이면 `keep_alive`·warm-up 호출 ([04 §3](04-parameters-and-context.md)) |
| **성공했는데 틀림** | 환각, 규칙 위반, 잘못된 도구 선택 | **HTTP 상태만으로는 판정 불가** — 검증 코드·골든셋·사람 | [05 §4](05-structured-output.md) 내용 검증, [08](08-lab-prompt-injection.md) 경계, §4 평가 |

`[해석]` 마지막 유형은 LLM 앱에서 특히 주의해야 합니다. HTTP 200은 요청이 처리됐다는 뜻이지, 답의 내용이 맞다는 뜻이 아닙니다.
...
400·401·404처럼 입력·인증·모델 설정의 문제는 원인을 고친 뒤 다시 요청합니다. 429(요청 제한)는 대기 후 재시도할 수 있으므로 모든 4xx를 같은 범주로 묶지 않습니다. 잘린·틀린 답도 원인을 확인합니다. `[해석]`

위 반복문은 최대 3번 `fn()`을 호출합니다. SDK 자체의 재시도와 겹치면 실제 HTTP 요청은 더 많아집니다. 이 반복문에서 횟수를 관리하려면 클라이언트에 `max_retries=0`을 설정합니다. [OpenAI Python SDK의 재시도 설정](https://github.com/openai/openai-python#retries)을 기준으로 확인했습니다. `[문서 확인 · 2026-09-13]`
...
잠깐 남긴 디버그 로그도 유출 경로가 될 수 있습니다. `[해석]`
