...
**가이드 작성 중 직접 확인한 검증 기록** — 본문에 실행 결과와 함께 수록되어 있습니다.

- 같은 모델이 선언한 context 상한은 262,144 토큰인데, 작성 환경에서 runtime이 적용한 창은 **4,096**이었습니다. 64배 차이를 `ollama ps`로 확인했습니다. (04 §2)
- "JSON으로만 답해"라고 프롬프트로 지시하자 완벽한 JSON 앞뒤에 ```` ```json ```` 펜스가 붙어 **파싱이 실패**했습니다. 스키마를 강제한 똑같은 요청은 성공했습니다. (05)
...
매번 같은 형식으로 남기려면 **[내 연결 카드를](APP-CARD.md)** 복사해 사용하세요. "응답이 왔다"에서 끝내지 않고
**무엇을 호출했는지(Call), 출력을 어떻게 고정했는지(Shape), 무엇을 막았는지(Guard)를** 함께 남기는 것이 이 가이드의 핵심 습관입니다.

## 먼저 구분할 세 가지

| 구분 | 뜻 | 흔한 혼동 |
| --- | --- | --- |
| **runtime** | 모델을 메모리에 올리고 HTTP로 요청을 받는 프로그램(Ollama, vLLM 등) | 모델과 runtime은 같은 것이 아닙니다. 같은 모델을 여러 runtime에서 실행할 수 있습니다 |
| **API(호환 형식)** | 요청·응답 JSON의 모양. 대부분의 runtime이 OpenAI의 형식을 흉내 냅니다 | "OpenAI 호환"이라고 성능이나 기능까지 같지는 않습니다. 기본적인 대화 방식만 같고 세부 부가 기능은 플랫폼마다 다릅니다 |
| **SDK·프레임워크** | 그 JSON을 대신 만들어 주는 라이브러리(openai SDK) / 여러 호출을 엮는 층(LangChain, Spring AI) | 프레임워크가 없어도 HTTP 요청 하나로 연결할 수 있습니다 |
...
이 가이드의 2부(06~08)는 두 번째 항목을 직접 확인하기 위해 **읽기 전용 도구**만 쓰고, 허용 범위를 넘는 시도를 일부러 만들어 봅니다.
...
이 가이드가 끝나는 지점은 **내 프로그램이 로컬 모델을 호출해 구조화된 출력을 받고, 읽기 전용 도구를 안전한 경계 안에서 쓰게 하며, 허용 범위를 넘는 시도를 멈출 수 있는 순간**입니다.
