...
산식으로 들어가기 전에 이것부터 본다. **컨텍스트 길이와 생성 속도를 이해하는 데 토큰이 필요하기 때문**이다.
...
`[원리]` 컨텍스트 길이는 **토큰 수**, 생성 속도(TPS)는 **초당 토큰 수**로 센다. KV 캐시의 메모리 크기는 모델 구조와 토큰 수를 바탕으로 바이트 단위로 계산한다.
...
> **"다음 토큰 하나를 고르는 일을 계속 반복한다."** 속도·메모리 이야기는 전부 이 반복의 비용이다. `[해석]`
...
| INT8 (8bit) | 1 byte | 7 GB | 70 GB | 품질 변화는 모델·작업별 확인 |
...
→ **실무 규칙: 계산값에 10~20% 여유를 더한다.** 이 가이드의 표는 일괄 +15%를 쓴다.
   이 여유는 예시용 추정치다. 4.8bpw라면 4bit보다 20% 크므로 +15%로 충분하다고 보장할 수 없다. 실제 파일 크기를 우선한다 — 상세는 [03 §4의 bpw 표](03-quantization.md). `[해석]`

### 이 숫자들을 어디서 보나 — 모델 스펙 카드 읽는 법

산식의 구조 변수는 주로 **모델 페이지의 `config.json`에서**, 총 파라미터 수 등은 모델 카드에서 확인한다. 모델 페이지 → `Files` 탭 → `config.json`을 연다. `[자료 확인 · 2026-08-10]`
...
```
→ **이 가정에서는 16GB 장비의 시험 후보.** OS·실제 파일 크기·KV와 실행 로그를 더 확인해야 한다. 8GB는 여유가 적어 그대로 실행된다고 보장할 수 없다.
...
**세 예제의 교훈:** 필요한 것은 결론이 아니라 **가중치·KV·실행 여유를 따로 계산하는 절차다.** +15%와 2GB는 위 예제의 가정이며 모든 모델과 런타임에 쓰는 고정 여유가 아니다. `[해석]`
...
> 하이브리드 SSM 계열 등은 **이 식보다 훨씬 작게** 나온다. 다른 구조에 이 식을 그대로 적용해 상한으로 단정하지 말고,
> 해당 구조의 계산법과 런타임이 출력하는 KV 할당량을 확인한다. `[변동]` 조회 2026-08-10
...
- 스펙의 숫자는 **모델이 선언한 지원 길이**다. 학습 길이와 확장 방식은 모델 카드에서 별도로 확인한다
...
→ **"긴 컨텍스트를 쓰고 싶다"와 "큰 모델을 쓰고 싶다"는 같은 메모리를 두고 경쟁한다.** 주어진 메모리 안에서 모델·양자화·컨텍스트를 함께 조정해야 한다. `[해석]`
...
| 증상 | 원인 |
| --- | --- |
| 답이 몇 줄 쓰다 뚝 끊긴다 | 생성 여지·출력 토큰 제한·중단 조건 등을 확인 |
| 긴 대화에서 앞 내용을 잊는다 | 앞부분 잘림·요약 또는 모델의 장문 정보 활용 실패 가능 |
