...
**4bit의 품질은 어떻게 지키나.** 실제 양자화는 **블록별 배율(scale)과 중요도 등을** 활용해 오차를 줄인다. 많은 파라미터의 오차가 자동으로 상쇄된다고 가정하지 말고, 원본이나 높은 정밀도의 모델과 실제 과제 결과를 비교한다. `[해석]`
...
> 표현할 수 있는 값이 적어 정보 손실 위험이 커진다. 영향의 크기는 모델과 방법에 따라 다르다.
...
여기서 **perplexity**라는 지표가 자주 인용된다 — 평가 텍스트의 실제 다음 토큰에 모델이 부여한 확률로 계산하는 값이다. 같은 평가 데이터와 토크나이저 조건에서 낮을수록 예측이 잘 맞는다([11 용어집](11-glossary.md)). 차이의 크기를 고정적으로 단정하지 않고 실제 평가값을 비교한다.
