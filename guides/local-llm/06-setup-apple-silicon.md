...
4,096이었습니다. 긴 컨텍스트가 필요하다면 기본값이나 모델 상한만 믿지 말고 실제 실행 값을 확인한 뒤 메모리 사용량을
...
- 모델 로드 후 앱에 표시되는 **메모리 사용량이 §1에서 정한 메모리 예산 안**에 들어온다
- 대화에 응답이 오고, 생성 중 Activity Monitor의 **GPU 사용률이 올라간다** (짧은 요청은 그래프에서 놓칠 수 있으므로 모델 설정과 적재 로그도 함께 확인)
...
| `--jinja` | 모델의 채팅 템플릿 적용 |
| `-ngl 99` | **GPU에 올릴 레이어 수.** 최대 99개를 요청. 전체 적재 여부는 모델의 레이어 수와 로그로 확인 |
