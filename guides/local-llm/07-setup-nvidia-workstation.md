...
| 플래그 | 의미 |
| --- | --- |
| `-ngl 99` | GPU에 올릴 레이어 수. 최대 99개를 요청하며 전체 적재 여부는 로그로 확인. VRAM이 모자라면 이 값을 낮춘다 |
| `-c 8192` | 컨텍스트 길이 |
...
**동시 요청이나 배치 처리가 없다면 더 단순한 경로부터 시작할 수 있다**([05 §3](05-stack-map.md)). 다음 중 하나에 해당할 때 의미가 있다.
