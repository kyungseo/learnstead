...
| 첫 응답만 느림 | prefill·cold load·queue 등 | 각 소요 시간과 서버 상태 확인 (§1) |
...
> `temperature`(토큰 선택의 확률 분포 조절), 구조화 출력(JSON 강제 — 런타임별 지원 편차 있음). `[자료 확인 · 2026-08-10]`
...
| "왜 처음 응답이 느리죠?" | prefill 외에도 model load·queue·network가 원인일 수 있습니다. 입력 길이와 서버 상태를 나눠 확인합니다 (§1) `[원리]` |
