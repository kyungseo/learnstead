...
갖춘 벤치마크가 아니므로, 이 값만으로 다른 장비나 모델과 성능을 비교하기는 어렵습니다.
...
## Markdown 렌더 검증 (2026-08-25)

가이드의 Markdown 문서 16개를 GFM 호환 parser인 Marked 17.0.5로 렌더했습니다.

| 확인 항목 | 결과 |
| --- | --- |
| source의 강조 구문과 렌더된 `<strong>` 수 비교 | 16/16 문서 일치 |
| inline code와 fenced code block 밖의 literal `**` | 0건 |
| 수정한 강조 구문 | 7개 문서 16곳 |

코드의 거듭제곱처럼 `**` 자체가 실행 문법인 경우는 Markdown 강조 검사에서 제외했습니다.
...
문서 검사를 통과했더라도 런타임 명령이 실제로 동작하는지는 직접 실행해 확인해야 합니다.
