...
>
> **읽고 나면:** 내 문서·내 질문에 RAG·Long Context·Fine-tuning 중 무엇이 맞는지 문서 규모·갱신·근거 추적 요구로 비교하고, context 창이 커져도 검색이 유용한 경우를 설명할 수 있습니다.
>
...
생길 때마다 재학습해야 하고, 가중치만으로는 어느 학습 문서가 근거인지 추적하기 어려우며, 학습 데이터 준비와 GPU 비용이 큽니다. 이 가이드는
...
| 상황 | 선택 | 이유 |
| --- | --- | --- |
| 문서 몇 편, 질문 몇 번 | Long Context | 별도 검색 없이 먼저 검증하기 쉬움 |
| 문서 수십 편 이상, 계속 늘어남 | RAG | 전체 입력 비용과 검색 품질을 비교할 시점 |
| "어느 규정 몇 조에 근거한 답인가"가 중요 | RAG | 검색된 조각 단위로 출처를 추적하기 쉬움 |
| 같은 문서에 질문을 수백 번 | RAG | 전체 입력 비용과 검색 비용을 비교. 입력 캐시 지원도 확인 |
