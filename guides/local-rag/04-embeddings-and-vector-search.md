...
- 임베딩은 텍스트를 **수백~수천 개의 숫자(벡터)로** 바꾼 것이고, 뜻이 비슷한 텍스트를 가까운 벡터로 표현하도록 학습합니다. `[원리]`
...
embedding model은 텍스트를 받아 고정 길이의 숫자 배열을 돌려주는 모델입니다. 예를 들어 bge-m3는 지원 입력 길이 안의
텍스트를 1,024개의 숫자로 표현합니다. `[문서 확인 · 2026-08-23]` 이 숫자들을 좌표로 보면, 텍스트 하나가
...
두 벡터의 가까움은 보통 **코사인 유사도**(두 벡터 사이 각도의 코사인)로 잽니다. 1에 가까울수록 같은 방향이고, 0이면 직교하며, 음수면 두 벡터 사이 각도가 90도를 넘습니다. 이 기하학적 관계가
문장의 의미가 무관하거나 정반대라는 판정을 보장하지는 않습니다. `[원리]`
...
무관 조각의 점수는 **겹치는 구간이 있고**, 이 실습에서 점수 하한(threshold)은 낮은 점수의 무관 조각을 걸렀지만, 점수가 높은 단어 겹침 조각까지
구분하지는 못했습니다. 내 데이터의 구간은 [실습 ③](../../labs/why-rag-fails/01-retrieval-failures.md)에서 직접 잡습니다. `[해석]`
...
`[문서 확인 · 2026-08-23]` — `api_key`는 이 예시의 Ollama 로컬 endpoint가 검증하지 않지만 SDK가 빈 값을 거부하므로 임의 문자열을 넣습니다.

> 🔧 **한 단계 더 — 질문용 접두어(instruction)**
>
> 접두어 규칙은 모델마다 다릅니다. Qwen3-Embedding은 질문에 작업 지시를 붙이는 방식을, multilingual-e5-large는 검색 시
> 질문에 `query: `, 문서에 `passage: `를 붙이는 방식을 안내합니다. bge-m3는 별도 접두어 없이 씁니다.
> 모델 카드의 usage 절에 맞춰 색인과 질의 입력을 각각 구성하세요. `[문서 확인 · 2026-09-13]`
