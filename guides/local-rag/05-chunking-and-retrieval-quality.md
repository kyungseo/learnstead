...
| **late chunking** | 지원 길이 안의 전체 텍스트를 토큰 벡터로 인코딩한 뒤, 조각별로 풀링해 대표 벡터를 만듦 | 조각이 문서 전체 문맥을 품음 | 지원 모델·도구 필요 | 상호 참조가 많은 문서 |

`[자료 확인 · 2026-08-23]` — 부모-자식은 검색 단위와 생성에 쓸 단위를 분리하는 방식이며, late chunking은 풀링 전에 조각 경계를 적용하는 방식입니다. 튜토리얼과 실습은
...
`[원리]` 두 방식이 **서로 다른 질문에서 실패하므로** 합친 결과를 같은 질문 묶음으로 비교하면 보완 효과가 있는지 확인할 수 있습니다. 이것이 hybrid입니다.
...
| 점수 하한(threshold) | 낮추면 관련 없는 조각이 섞일 수 있음 | 높이면 무관한 조각과 함께 정답도 차단될 수 있음 |
...
>
> 재작성·multi-query·HyDE는 호출이나 검색을 추가합니다. 메타데이터 필터는 검색 대상을 줄이지만, 필터 조건과 인덱스에 따라 비용이 달라집니다. 청킹·hybrid·reranker를 먼저 정리한 뒤에 손댑니다. `[해석]`
...
- [ ] 규칙·조각·질문·답변 여유를 합쳐 적용 context 창 안에 들어가는가
