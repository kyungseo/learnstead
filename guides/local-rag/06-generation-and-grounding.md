...
>
> **읽고 나면:** 규칙 → 근거 → 질문으로 프롬프트를 조립하고, 번호 인용을 요구하고 원문과 대조해 사람이 검증할 수 있는 답을 받으며, context 예산을 계산할 수 있습니다.
>
...
- "근거에 없으면 모른다고 답하라"와 "근거 번호를 인용하라"는 두 지시가 추가 도구 없이 먼저 적용해 볼 지시입니다. 효과는 실제 답과 근거를 대조해 확인합니다. `[해석]`
...
| "근거만으로" | 근거 밖 내용을 쓰지 않도록 지시 | 근거와 무관한 지어낸 답 |
...
이 경우의 중단 여부는 코드가 결정합니다. 다만 높은 점수의 무관 조각은 통과할 수 있어, 하한만으로 근거 충실도를 보장하지는 못합니다. `[해석]`
