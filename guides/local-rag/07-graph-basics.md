...
| 출처 | 방법 | 비용·정확도 |
| --- | --- | --- |
| 이미 구조화된 데이터 | DB 테이블·조직도·제품 카탈로그를 노드·엣지로 변환 | 추출 비용을 줄일 수 있음. 원본과 변환 규칙의 품질 확인 |
| 문서(비구조) | **LLM이 문장에서 트리플을 추출** | 비싸고(조각마다 LLM 호출) 오류 섞임 |
| 사람 | 직접 작성·검수 | 시간과 검수 기준 필요. 누락·오입력 확인 |
...
관계 탐색과 원문 검색이 모두 필요한 경우 벡터와 그래프를 함께 쓸 수 있습니다. 다음 문서가 그 결합을 다룹니다.
