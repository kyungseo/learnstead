...
[← 이전: 08 — GraphRAG: 언제 그래프가 필요한가](08-graphrag.md) · [목차](README.md) · [다음: 10 — 용어집](10-glossary.md)
<!-- learnstead:nav:end -->
...
| **권한** | 문서별 ACL(접근 권한 목록)을 조각 메타데이터로 동기화, 질의 시 사용자 권한으로 **검색 전** 필터 | 없음 (내 문서는 다 내 것) | 인사 문서가 일반 직원 답변에 섞이면 안 됨. 생성·외부 재정렬·로그에 넘기기 전에 권한 확인 필수 |
...
1. **검색 전 필터(pre-filter):** 사용자가 볼 수 있는 문서의 조각만 검색 대상에 넣습니다. 검색 후 필터는 후보가 줄어 근거가 부족해질 수 있습니다. 어느 방식을 쓰든 권한 없는 조각을
   모델·외부 재정렬 서비스·사용자용 로그에 전달하기 전에 반드시 차단해야 합니다.
...
로 거르면 됩니다. 이는 단일 그룹만 다루는 학습용 예입니다. 실제 운영에서는 다중 그룹·명시적 거부·권한 철회·캐시와 **동기화 누락을** 함께 처리해야 합니다.
...
원칙: **검색과 생성을 따로 잽니다.** 종단 점수만 보면 "왜 나빠졌는지"를 알 수 없습니다. 청킹 변경 뒤 recall이 떨어지면 조각 경계와 정답 ID 매핑을 먼저 확인합니다. recall이 같아도 가져온 잡음이나 근거 순서가 달라질 수 있으므로,
faithfulness 하락은 프롬프트·모델뿐 아니라 실제 검색 결과와 함께 살펴봅니다.
...
튜토리얼 이후에 필요한 부품 — 인증 연동, 게이트웨이, DLP·마스킹, 감사 로그 보존, 규제 대응 — 은 RAG가 아니라 **조직의 AI 운영 체계의**
