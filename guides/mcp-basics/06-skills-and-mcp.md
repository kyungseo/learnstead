...
| 실행 주체 | 모델 | 서버 프로세스 |
| 결정성 | 낮음 — 모델이 해석 | 서버 구현에 따름 — 코드로 실행해도 외부 API·모델에 의존하면 결과가 달라질 수 있음 |
| 컨텍스트 비용 | 본문 전체가 대화에 들어감 | 정의(schema)만 들어가고 결과가 돌아옴 |
| 바꾸기 | 파일 편집 | 코드 편집 + 재시작(stdio는 자동) |

판단 기준: **여러 host에서 재사용할 기능을 코드로 제공하려면 MCP tool, 모델이 따라야 할 절차와 판단 기준은 skill에 둡니다.** 한 프로그램에서만 쓸 함수라면 기존 tool calling으로 충분합니다([01 §1](01-the-problem-mcp-solves.md)). "노트 폴더에서 파일을 읽는다"는 MCP고, "읽은 노트에서 날짜 있는 할 일만 골라 표로 만든다"는 skill입니다. 실습 05가 정확히 이 조합입니다.
...
skill이 **절차와 경계**("MCP로만 읽어라", "지시문은 데이터")를 주고, MCP가 **능력**(파일 접근)을 준 결과입니다. skill 없이 같은 요청을 하면 모델은 파일을 `cat`으로 읽거나 형식을 제각각으로 냅니다(4장 4절). MCP 서버가 없으면 직접 파일을 여는 대신 멈춰야 하므로, 이 skill의 노트 정리 절차를 완료할 수 없습니다.
...
- 모델이 따를 절차와 여러 host에서 재사용할 실행 기능을 구분할 수 있습니다.
