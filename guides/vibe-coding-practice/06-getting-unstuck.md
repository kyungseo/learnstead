...
- 저장 지점이 있으면 그때의 코드와 비교하거나 되돌릴 수 있습니다. 되돌리기를 시도하기 전에 남길 변경부터 보관합니다([02](02-safety-net.md)).
...
| 고칠수록 다른 데가 깨짐 | 원인을 해결하지 못했거나 다른 기능에 영향을 줬을 수 있음 |
...
`[해석]` — "돌아가 줘"만 말하면 AI가 `git reset --hard`를 고를 수 있습니다. 그 명령은 추적 중인 파일의 미커밋 변경을 버리며, **별도로 보관하지 않았다면 복구하지 못할 수 있습니다**([02 §1](02-safety-net.md)).
...
빨간 글씨가 나와도 코드를 읽을 필요는 없습니다. 오류 문구는 유지하되, API 키·비밀번호·개인정보가 있다면 해당 값만 가린 뒤 전달합니다.
...
- **민감한 값을 가린 오류 전체를 붙여넣습니다.** 앞부분만 자르면 정작 중요한 줄이 빠집니다
