...
| 도구 | 구성 | 총 경과 시간 중앙값(초) | 메인 입력 | 위임 입력 | 테스트 | 바뀐 파일 수 |
| --- | --- | --- | --- | --- | --- | --- |
| Claude Code | 동시 3 | | | | | |
| Claude Code | 순차 | | | | | |
| Codex | 동시 3 | | | | | |
...
| 도구 | 구성 | 총 경과 시간 중앙값(초) | 메인 컨텍스트(끝) | 메인 누적 | 위임 누적 | 테스트 |
| --- | --- | --- | --- | --- | --- | --- |
...
| 증상 | 원인 | 복구 |
| --- | --- | --- |
| 동시 3의 총 경과 시간이 순차와 같다 | 순차 실행·대기·요청 한도 등 여러 가능성 | 스트림에서 `Agent` 호출 시각을 보고 실제 동시성 확인. 겹치지 않았다면 그 사실도 결과에 기록 |
