...
서로 다른 파일을 고치는 과제 셋을 subagent 셋에 동시에 맡긴 벽시계를 순차와 대조하고, 같은 함수를 고치는 과제 둘을 격리 없이 · worktree로 돌려 최종 파일과 merge를 본다.
...
| 증상 | 원인 | 복구 |
| --- | --- | --- |
| 동시 3의 벽시계가 순차와 같다 | 순차 실행·대기·요청 한도 등 여러 가능성 | 스트림에서 `Agent` 호출 시각을 보고 실제 동시성 확인. 그것도 관측 |
| worktree 실행 뒤 `proj/`에 변경이 없다 | 편집이 `.claude/worktrees/<이름>/`에 남음 | `worktrees.txt`로 위치 확인. 메인이 합쳤는지가 관측 대상 |
