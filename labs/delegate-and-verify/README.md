...
> 작은 가계부 프로젝트에 결함 세 개를 심어 두고, 같은 과제를 **한 세션 · subagent 위임 · subagent 병렬 · 새 컨텍스트 검토로** Claude Code와 Codex CLI에서 반복 실행합니다. 메인 컨텍스트 크기·벽시계·결함 적중을 채점기로 재고, 위임 프롬프트 결핍·같은 파일 충돌·검토 과잉을 재현하며, 작성 세션 이어 가기 검토와 새 컨텍스트 검토를 같은 기준으로 비교합니다. 가이드 [AI Agent에게 일을 나눠 맡기는 법](../../guides/agent-delegation/README.md)의 실습편.

## 학습 달성 목표(Learning Objective)

- 한 세션의 기준선(토큰·시간·적중)을 만들고 그 위에서 나누기의 효과를 판정한다.
- subagent에 위임했을 때 메인 컨텍스트가 줄었는지, 전체 토큰이 늘었는지를 도구의 usage 기록에서 읽는다.
- 병렬이 실제로 시간을 줄였는지 순차와 대조하고, 같은 파일 편집의 충돌을 worktree 유무로 재현한다.
- 작성 세션 이어 가기 검토와 새 컨텍스트 검토, "결함만"과 "모든 gap" 프롬프트의 적중과 골든셋 밖 지적을 같은 자로 잰다.
...
4. 04에서 이어 가기·새 컨텍스트·교차 검토의 지시 위반 적중과 검토 턴 비용, 두 검토 프롬프트의 골든셋 밖 지적 수를 기록한다.
