...
1. 01에서 라운드 1(추론 가능)을 최소 V0·V1로 돌려 실제 점수를 기록한다.
2. 02에서 규칙을 따르지 않는 원본 fixture로 V0·V1을 돌려 라운드 1과 비교한다.
...
실행이 중간에 실패하면 `runs/<라벨>/`에 일부 파일이 남을 수 있습니다. 원인을 확인한 뒤 같은 라벨로 다시 실행하면 경로 검사를 거쳐 해당 디렉터리의 기존 파일을 지우고 새로 만듭니다. 보존하고 싶다면 다른 라벨을 사용합니다.
...
작성 환경에서는 V1·V3i·V4(Claude)의 점수가 비교적 높았습니다. 아래는 당시 관측한 결과이며, 재현 시 같은 점수가 나와야 완료되는 실습은 아닙니다.
...
원본 fixture와 사용자 지시문은 직접 수정하지 않습니다. 실습 파일은 홈 디렉터리 아래에 생기며, CLI가 자신의 설정·세션 기록을 추가할 수 있습니다. 사용자 레벨 `~/.claude/CLAUDE.md`·`~/.codex/AGENTS.md`가 있으면 실험에 함께 로드되므로 결과를 읽을 때 감안합니다.
