...
해석 [해석]: description은 "이 skill을 열까"의 근거일 뿐이고, 열린 뒤 무엇을 할지는 본문의 내용과 사용자 요청·우선순위 규칙이 함께 영향을 준다. 넓은 description의 비용은 **불필요한 로드(토큰)와 모순으로 인한 혼란이지**, 항상 잘못된 출력은 아니다. 그래서 과호출은 출력만 봐서는 잘 안 보이고 로그의 tool 호출을 봐야 한다.
