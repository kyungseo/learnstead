...
2. 02에서 description 변경 전후의 호출 여부를 로그로 비교한다.
...
Claude Code만 있어도 01·02·05는 진행할 수 있다. Codex만 있다면 03의 Codex 호출과 경로 관측부터 진행한다. 03·04의 도구 비교는 둘 다 필요하다.
...
01·04·05의 skill on은 판정 PASS를 목표로 한다. 아래는 작성 환경에서 관측한 비교 결과다. 재현 결과가 다르면 그 차이를 기록한다:
