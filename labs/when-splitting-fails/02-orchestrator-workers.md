...
문서별로 모델을 호출하는 코드를 봅니다. 이 실습은 문서 하나당 워커 하나를 코드가 고정한 fan-out/fan-in이며, 모델이 작업을 동적으로 분해하는 구성은 아닙니다. 각 워커의 **입력 경계는** 전달 자료로 확인합니다. 그런데 병렬로
...
단일 호출로 하면 전부를 한 프롬프트에 넣어야 하고, 대상이 늘수록 컨텍스트 창을 넘습니다. `[원리]`
...
두 층에서 틀렸습니다. 휴가 워커가 "가산"과 "이월"을 한 문장으로 엮어 **규정에 없는 인과**("가산하여 이월할 수 있으며")를 만들었고,
...
| 원인 부품 | 취합 (여러 결과를 한 문장으로 합치는 단계) — 워커가 "모른다"고 한 것도 결과로 들어감 |
| --- | --- |
...
Ollama는 `OLLAMA_NUM_PARALLEL`을 지정하지 않으면 메모리에 따라 동시 처리 수를 정하며, 이 환경에서는 사실상 1이었습니다.
`[실행 검증 · 2026-08-30]` `[문서 확인 · 2026-08-30]`

> **초판과 다른 점:** 2026-08-23판은 "순차라면 13초였을 것"이라고 추론만 하고 병렬이 됐다고 적었습니다. 대조 측정을 하니 아니었습니다.
> **병렬은 코드가 아니라 runtime이 결정합니다.** 스레드를 셋 띄운 것과 셋이 동시에 계산되는 것은 다른 일입니다. `[해석]`

병렬을 실제로 얻으려면 서버 쪽 동시 처리(`OLLAMA_NUM_PARALLEL=3` 등)를 켜야 하고, 그러면 메모리가 그만큼 더 듭니다. 2026-09-06에
...
"모른다"고 답하며, 상위가 그 셋을 다시 읽습니다. `[실행 검증 · 2026-08-30]`

**언제 이득인가** `[해석]`:

| 조건 | 이유 |
| --- | --- |
| 대상이 **많다** (문서 10편+) | 단일 호출로는 컨텍스트 창을 넘음 |
| 대상이 **서로 독립적이다** | 워커는 옆 워커를 모름 |
...
| **병렬이 아님** | 워커 시간이 계단 (§3) | 순차와 대조 측정, runtime 동시 처리 설정 확인 |
| **워커 하나가 실패** | 예외로 전체 중단 | 워커별 예외 처리 후 "실패"도 결과로 취합 |
| **취합이 원문을 다시 봄** | 추가 입력 발생 | 검증에 필요한 원문 범위와 비용 확인 |

> 🔧 **한 단계 더 — 부분 실패를 살려 내기**
>
> `ex.map`은 예외가 나면 그 자리에서 터집니다. 실무에서는 워커마다 `try/except`로 감싸고 실패를 `("워커명", "실패: 사유")`로
