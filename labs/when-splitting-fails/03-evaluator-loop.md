# 03. 평가-개선 루프 — 객관적 기준은 1회에 끝나고, 주관적 과제는 끝나지 않는다
...
8.5배** 썼습니다. 그리고 "같은 지적이 반복되면 멈추기"는 **발동하지 않았습니다.** 왜 그런지가 이 문서입니다.
...
**평가자도 같은 모델입니다.** 작성자가 못 본 것을 평가자가 볼 이유가 구조적으로 없습니다. 그리고 "문제가 있는가?"라고 물으면 LLM은
**찾아내는 쪽으로 기웁니다** — 없다고 말하는 것보다 뭐라도 지적하는 것이 "성실한 답변"처럼 보이기 때문입니다. `[해석]`
...
| 원인 부품 | 평가 기준 (모호한 기준을 같은 모델에게 판정시킴) |
| --- | --- |
...
| **판정이 이분법** | "품질이 좋은가" | "JSON이 스키마에 맞는가" |
...
여부는 전부 코드로 확인할 수 있고 그게 훨씬 싸고 정확합니다.
