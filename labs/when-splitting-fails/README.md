...
> "AI를 여러 개 붙이면 더 똑똑해진다"는 말을 **같은 질문, 같은 모델, 같은 문서 3편으로** 직접 검증합니다. 단일 호출·파이프라인·라우터·오케스트레이터-워커·평가 루프 다섯 방식을 한 스크립트로 돌려 호출 수·토큰·시간·정답 여부를 나란히 놓고, 나눴을 때 생기는 실패(오분류 전파·조용한 손실·수렴 실패·비용 폭발)를 일부러 재현한 뒤 고치는 장치가 실제로 결과를 바꾸는지 봅니다. 가이드 [여러 AI를 엮어 일하게 하기](../../guides/agent-orchestration/README.md)의 실습편이며, 가이드 없이도 실행됩니다. 모델을 프로그램에서 부르는 법과 구조화 출력은 [앱 연결 가이드](../../guides/local-llm-app-integration/README.md)를 전제로 합니다.

## 학습 달성 목표(Learning Objective)

- 다섯 방식을 같은 자(호출 수·입력 토큰·시간·정답)로 잰다.
- 각 호출의 전달 자료로 입력 경계를 확인하고, 전체 토큰을 기준선과 별도로 비교한다.
- 오분류 전파·조용한 손실·수렴 실패를 재현하고 출력의 어느 줄로 판정하는지 안다.
- 폴백·순차 대조·토큰 예산 한 가지로 결과가 바뀌는 것을 보고, 기준선과 비교해 유지·되돌림을 판정한다.
...
모든 시나리오에서 보는 것은 네 줄입니다. `[원리]`

1. **기준선** — 같은 질문을 단일 호출로 돌린 호출 1회·입력 토큰·시간·정답. 이것 없이는 아무것도 판정할 수 없습니다.
...
코드는 `ask()` 하나만 모델을 부르고, 패턴의 차이는 **"언제 몇 번 부르는가"뿐입니다.** 모든 호출이 `[단계] 시간 · 입력→출력 토큰 · 출력 앞부분`을
...
이 실습은 파일을 만들지 않습니다. 모델을 내리려면 `ollama stop gemma3:4b`. 기록은 [오케스트레이션 카드](../../guides/agent-orchestration/ORCHESTRATION-CARD.md)에 "기준선 → 나눈 구성 →
유지/되돌림"으로 남깁니다.
...
| # | 문서 | 내용 |
| --- | --- | --- |
| 01 | [파이프라인과 라우터](01-pipeline-router.md) | 기준선 · 격리로 토큰이 주는 장면 · ① 오분류 전파 재현과 8문항 정확도 · 폴백으로 되돌리기 |
...
이 실습이 끝나는 지점은 **다섯 방식의 숫자를 한 표에 놓고, 나눈 구성의 실패를 로그에서 짚어 내며, 한 가지 장치로 결과가 바뀌는 것을 본 순간입니다.**
