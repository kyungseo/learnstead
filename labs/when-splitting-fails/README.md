...
- 각 호출의 전달 자료로 입력 경계를 확인하고, 전체 토큰을 기준선과 별도로 비교합니다.
- 오분류 전파·조용한 손실·수렴 실패를 재현하고 출력의 어느 줄로 판정하는지 확인합니다.
- 실패 시 대체 경로(폴백), 순차 실행과의 비교, 토큰 사용량 제한을 각각 적용하고, 기준선과 비교해 유지·되돌림을 판정합니다.

## 완료 조건

1. `patterns.py all`로 다섯 방식의 호출 수·입력 토큰·시간·정답을 한 표에 적었다.
2. 라우터 8문항 `--batch` 정확도와 `--fallback` 비용을 기록했다(01).
3. 워커 병렬과 `--max-workers 1` 순차의 총 경과 시간을 대조하고, 워커 결과와 취합 답을 대조했다(02).
