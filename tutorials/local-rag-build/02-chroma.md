...
조각이 늘면 매번 임베딩을 다시 계산하고 전부 비교하는 비용이 커집니다. 색인을 파일에 저장하고 가까운 것만 찾아 주는 vector store를
...
**★ 성공 판정:** `색인 완료: 17개 조각`이 출력되고, 질문의 근거 상위가 01 §3과 같은 조각(#15·#16·#14)입니다. 점수는 이 스크립트가 Chroma의 코사인 거리를
...
`색인을 삭제했습니다.`가 출력되면 이 실습의 Chroma 색인이 삭제된 것입니다. 원본 문서와 설치한 패키지·모델은 남습니다. `[실행 검증 · 2026-08-30]`
