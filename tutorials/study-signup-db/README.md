...
> **실행 검증 · 2026-09-13:** macOS·Node.js 24·Chrome에서 설치부터 로그인·신청·메모·계정 전환·중지·재시작·초기화까지 확인했습니다. 실제 버전과 브라우저 동작 판정 11개, DB 통합 검증의 구분은 [검증 기록](VALIDATION.md)에 있습니다.
...
실제로 확인한 OS·도구 버전·명령·결과는 [검증 기록](VALIDATION.md)을 기준으로 읽으세요. 이 실습은 macOS 로컬 환경에서 확인했습니다. Windows 환경에서의 실행, 클라우드 배포, 실제 서비스 운영은 별도로 검증하지 않았습니다.
...
| `npm run setup` | 가상 계정·브라우저용 공개 설정 준비. 기존 신청 보존 |
